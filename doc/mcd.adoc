...
[#cli_mcd_rtl_xcelium,reftext='{listing-caption} {counter:refnum}']
[source, bash]
----
...
[#listing_mcd_32_logic_levels,reftext='{figure-caption} {counter:refnum}']
[%unbreakable]
[source, tcl]
----
...
Le metriche ottenute dalla sintesi di `mcd` per i valori di `OPSIZE => {16,32,64}` sono presentati in tabella, dove il report sul consumo di potenza é basato sull'attivitá di switching ottenuta dalle simulazioni RTL (copertura driver ~26% #TODO#).

[%unbreakable]
[table_mcd_syn_opt_results,subs="attributes+"]
...
[gnuplot]
....
set title "Design metrics vs Parameter"
set xlabel "Parameter size"
set xtics ( "16" 1, "32" 2, "64" 3 )
