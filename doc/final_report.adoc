...
[abstract]
== Abstract
Questo report presenta l'analisi e la simulazione di quattro dispositivi VHDL osservati durante il corso *Calcolatori Elettronici 2025*.
Vengono descritte le metodologie di test RTL e post-sintesi, i risultati di timing e utilizzo risorse, e vengono presentate considerazioni su questi.
Infine, si evidenziano limiti sperimentali e potenziali sviluppi futuri.
...
[WARNING]
====
Le immagini ottenute tramite screenshot incluse in questo documento sono state catturate con attenzione per garantire la migliore qualità possibile.
Si consiglia di consultare la versione digitale del documento per una resa ottimale di tali immagini.
====
