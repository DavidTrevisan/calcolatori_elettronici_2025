...
Questa modifica presenta peró delle peculiaritá nel caso `MEM_LAT = 1`.
Il componente infatti si comporta come segue al set del segnale `MEM_ENABLE`:

. **Al ciclo N**: `latcnt` viene impostato, ed i segnali `R*` vengono catturati
. **Al ciclo N+1**: `latcnt = 0` ed i segnali registrati vengono usati.

Questo crea a tutti gli effetti un _ritardo_ di un ciclo anche nel caso di `MEM_LAT = 1` quando invece si desidera la disponibilitá immediata del dato.

Per garantire il comportamento identico all'originale nel caso `MEM_LAT => 1`, sono stati utilizzati dei costrutti `if...generate` per sovrascrivere _l'extra buffering_ dei segnali in questo caso.
...
Si considera che questa versione modelli una memoria a latenza variabile secondo le richieste e che il comportamento sia assimilabile ad un'implementazione tecnologica equivalente.
...
Si nota che, per ogni transazione con la memoria esterna, un ciclo viene 'sprecato' tra la preparazione dell'indirizzo ed il segnale di enable, mentre si attende la disponibilitá del dato.
...
In questa versione, il dispositivo effettua entrambe le operazioni di `FETCH` e `COMPARE` in un unico stato e quindi prepara continuamente la prossima richiesta verso la memoria.
Il collo di bottiglia é dato perció dalla memoria stessa, ma ad un leggero incremento di percorsi combinatori nella control unit.
...
NOTE: Trovato un possibile baco funzionale nel modello di memoria a latenza variabile. Questo porta i problemi successivi. Suggerisco l'uso di un semplice shift register impostato da `MEM_LAT` per trasportare il dato al ciclo desiderato.
