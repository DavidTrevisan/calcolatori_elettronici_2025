...
Error   : LEF File Interface. [PHYS-100] [set_db]
        : File '/home/david.trevisan/03_multiplier_and_clz/synth/libs/stdcells.lef' contains an error on or near line '9669'.
...
Le metriche risultanti sono raccolte nei rapporti salvati sotto `synth/output`, o possono essere generate nuovamente tramite gli script di sintesi.
...
Analizzando il file HDL con questa modifica, appare correttamente l'aggiunta del circuito di clock gating, con il segnale controllato `rc_gclk` assegnato ai bit dei registri indicati prima:
...
Se si desiderasse mantenere questa configurazione, occorrerebbe aggiustare il circuito di gating permettendo il controllo del pin di clock dei registri quando in modalitá `TEST`.
