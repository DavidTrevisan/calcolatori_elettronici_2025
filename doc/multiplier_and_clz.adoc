...
Il modulo è chiamato `BUF_X8`, intermedio tra `BUF_X1` e `BUF_X32`, che lo script imposta con i comandi:
...
seguito dalla capacità di uscita media impostata al valore scalato (Genus utilizza `pF` mentre la libreria utilizza `fF`) preso dal campo `BUF_X8.pin (A).max_capacitance`:
...
.Comando correzione DFT per clock-gating
[source, tcl, reftext='{listing-caption} {counter:listnum}']
----
...
[source,shell, reftext='{listing-caption} {counter:listnum}']
----
...
.Stime dissipazione di potenza `device`
